...
namespace HTTPnet.Core
{
    ...
    {
        private readonly Func<HttpClientOptions, IClientSocketWrapper> _socketWrapperFactory;

        private IClientSocketWrapper _socketWrapper;
        private HttpClientOptions _options;
        private CancellationTokenSource _cancellationTokenSource;

        public HttpClient(Func<HttpClientOptions, IClientSocketWrapper> socketWrapperFactory)
        {
        ...
        public async Task StartAsync(HttpClientOptions options)
        {
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        public async Task StopAsync()
        {
            ...
            {
                await _socketWrapper.DisconnectAsync();
            }
        ...
        private async Task AcceptConnectionsAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    //var client = await _socketWrapper.SendStream.().ConfigureAwait(false);
